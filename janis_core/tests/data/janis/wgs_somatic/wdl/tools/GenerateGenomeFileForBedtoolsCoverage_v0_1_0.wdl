...
task GenerateGenomeFileForBedtoolsCoverage {
  input {
    Int? runtime_cpu
    Int? runtime_memory
    Int? runtime_seconds
...
cat <<EOT >> GenerateGenomeFileForBedtoolsCoverage-script.py
...
EOT
    python GenerateGenomeFileForBedtoolsCoverage-script.py \
      --reference '~{reference}' \
      ~{if defined(select_first([output_filename, "genome_file.txt"])) then ("--output_filename '" + select_first([output_filename, "genome_file.txt"]) + "'") else ""}
  >>>
  runtime {
