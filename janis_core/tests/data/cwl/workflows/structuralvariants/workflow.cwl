...
steps:
  bwa_index:
    run: subworkflows/bwa_index.cwl
    in:
  ...
  trimmed_fastq:
    run: subworkflows/trimmed_fastq.cwl
    in:
      fastq1: fastq1
      fastq2: fastq2
  ...
  samtools_view_sam2bam:
    run: subworkflows/samtools_view_sam2bam.cwl
    in:
      paired:
  ...
  samtools_sort:
    run: subworkflows/samtools_sort.cwl
    in:
      paired:
  ...
  picard_markduplicates:
    run: subworkflows/picard_markduplicates.cwl
    in:
      input:
  ...
  bam_filtering:
    run: subworkflows/bam_filtering.cwl
    in:
      input:
  ...
  cnv_manta:
    run: subworkflows/cnv_manta.cwl
    in:
      bams:
  ...
  cnv_gridss:
    run: subworkflows/cnv_gridss.cwl
    in:
      bams:
        source: bam_filtering/output
      samples: samples
  ...
  cnv_exomedepth:
    run: subworkflows/cnv_exome_depth.cwl
    in:
      bams:
        source: bam_filtering/output
      samples: samples
  ...
  cnv_codex:
    run: subworkflows/cnv_codex.cwl
    in:
      bams:
        source: bam_filtering/output
      samples: samples
  ...
  final_filtering:
    run: subworkflows/final_filtering.cwl
    in:
      manta_input:
...
s:codeRepository: https://gitlab.bsc.es/lrodrig1/structuralvariants_poc.git # TODO: move to INB general project
