...
class ConnectionInputIngestor:
    ...
    def ingest_connections(self, g_step: dict[str, Any]) -> None:
        ...
        for g_target, g_emitter in g_step['input_connections'].items():
            ...
            j_target = internal_mapping.tool_input(g_target, j_step.tool)
            j_emitter = internal_mapping.emitter(g_emitter['id'], g_emitter['output_name'], self.janis, self.galaxy)
            ...
            if isinstance(j_emitter, WorkflowInput):
                ...
                j_emitter_step = internal_mapping.step(g_emitter['id'], self.janis, self.galaxy)
