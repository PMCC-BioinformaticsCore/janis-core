...
class HailBatchTranslator(TranslatorBase):

    generate_click_cli = False
    name = "Hail batch"
...
""", tool
    ...
    def stringify_translated_workflow(cls, wf_intermediate: Tuple[str, WorkflowBase]) -> str:
        ...
        if cls.generate_click_cli:
            function_name = "main_from_click"
            extra_imports.append("import click")
            name_equals_main_arg = f"""\
{cls.generate_click_function(wf.tool_inputs(), to_call="main", click_function_name=function_name)}
...
{name_equals_main_arg}
        ...
        try:
            ...
            #   'black' in try block with 'except ImportError' should also be defined in except block
            black = None
            Logger.debug(
    ...
    @classmethod
    def stringify_translated_tool(cls, tool_intermediate: Tuple[str, Tool]) -> str:
        extra_imports = []
        tool_str, tool = tool_intermediate

        if cls.generate_click_cli:
            function_name = "main_from_click"
            extra_imports.append("import click")
            name_equals_main_arg = f"""\
        {cls.generate_click_function(tool.tool_inputs(), to_call="main", click_function_name=function_name)}
if __name__ == "__main__":
    b = hb.Batch("{tool.id()}")
    {function_name}(b=b)
    b.run(dry_run=True)
...
if __name__ == "__main__":
    b = hb.Batch("{tool.id()}")
    main(b, {", ".join(k + "=None" for k in kwargs)})
    b.run(dry_run=True)
        """
        extra_imports_str = "\n" + "\n".join(extra_imports)
...
{name_equals_main_arg}
    ...
    def stringify_translated_inputs(cls, inputs):
        return JanisTranslator.get_string_repr(inputs)

    @classmethod
    def workflow_filename(cls, workflow):
        return f"{workflow.versioned_id()}_batch.py"

    @classmethod
    def inputs_filename(cls, workflow):
        return f"{workflow.versioned_id()}_inputs.py"
    ...
    def resources_filename(cls, workflow):
        return cls.inputs_filename(workflow)
